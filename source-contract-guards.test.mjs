...
// Source guards only: these tests detect integration drift during refactors;
// they do not execute Intelligent Contract behavior or prove runtime outcomes.
...
test('frontend wires a blocking exact-wallet confirmation before grant signing', () => {
  assert.match(html, /id="confirmGranteeRisk"/)
  assert.match(html, /id="submitGrant" disabled/)
  assert.match(app, /submitOnlyAfterGrantConfirmation/)
  assert.match(app, /No transaction was sent/)
})
